...
function Jacobian(t::Int64, δ_jt::Vector{Float64}, σ_j::Vector{Float64})
    #we need to get the gradient which requires using to the the matrix σ of choice probabilities for product j from individual r 
    @unpack r, λ_p, markets = prim 
    t_ind = findall(x-> x == markets[t], car_characteristics[:, 2])  #finding all the products for market t = 1985
...
function Newtons_method(prim::Primitives, t::Int64)
    t_ind = findall(x-> x == markets[t], car_characteristics[:, 2]) 
    #1. define a function that minimizes the difference between the new and old market shares f = ln(true market shares) - ln(simulated market shares)
    δ_jt = contraction_mapping2(prim, t)
    σ_j = market_shares(t, δ_jt)  #computing new market shares 
    σ_0 = log.(Vector{Float64}(car_characteristics[:, 4][t_ind]))    #computing old market shares 
    f_t = σ_0 - σ_j 
     
    #iterate until f_t < tol 
    tol = 1e-12
    it = 0
    while maximum(abs.(f_t)) > tol 
        it += 1
        println("on iteration", it) 
        #2. Get the Jacobian 
        Df = Jacobian(t, δ_jt, σ_j)
        #3. Update the market shares 
        δ_jt_1 = δ_jt - inv(Df)*f_t 
        δ_jt = δ_jt_1   #updating the mean utilities
        σ_j = market_shares(t, δ_jt)  #computing new market shares
        f_t = σ_0 - σ_j   #updating the difference
        println("the difference is now  ", f_t)
    end
end

#Now we combine the two. First, we'll use contraction mapping until the error is 1, then have the function return the current δ_jt. 
# Using that δ_jt, we will then apply Newton's method 

function contraction_mapping2(prim::Primitives, t::Int64)
    @unpack r, λ_p, markets = prim 
    t_ind = findall(x-> x == markets[t], car_characteristics[:, 2])  #finding all the products for market t 
    #Write a routine that inverts the demand fucntion for parameter value λ_p = 0.6
    # the initial guess of the mean utilities is the delta_iia column in the characteristics data set 
...
Newtons_method(prim, 31)
#Took 56 iterations for the contraction mapping part and 4 iterations for the Newton's Method part for a total of 60 iterations 
